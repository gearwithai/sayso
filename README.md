...
## Call it anything
Setup step 3 (or Settings → **Its name → Change & test**): type a name like *Jarvis* or *Nova*, say *"Jarvis, hello"*, and Sayso shows **"Jarvis is ready to drive!"** without typing anything. Two-syllable names work best; very common words ("hey", "okay") are refused so it doesn't wake by accident.

## Limits
- **No usage limits.** No account, no quota, no time limit. It runs on your PC, so 2 hours or 24 hours of use is the same.
- **One phrase** can be up to 45 seconds; longer speech is typed in chunks. Dictation mode switches itself off after 2 quiet minutes.
- **Your computer does the work.** It only transcribes when it hears speech, so in a quiet room it uses almost nothing; with a TV or meeting in the background it works harder (use Pause, or hold-to-talk, during calls).
- **AI actions** are unlimited with Ollama/LM Studio on your PC; with your own OpenAI/Claude/Groq key, that provider's limits and prices apply.

## Updating
Run the new `SaysoSetup.exe` over the old one - no need to uninstall. It closes the running Sayso, replaces the program and keeps your settings, words, snippets and name. Sayso tells you (by the clock) when a new version is out.
