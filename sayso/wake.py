...
COMMON_WORDS = {"hey", "hi", "hello", "okay", "ok", "yes", "yeah", "no", "so", "say", "the", "and", "right",
                "well", "what", "like", "now", "go", "stop", "please", "thanks", "sorry"}


def name_advice(name: str):
    """Checks a wake word the user picked. -> (ok, message). ok=False means don't use it."""
    letters = _letters(name)
    if len(letters) < 3:
        return False, "Use a name with at least 3 letters."
    if letters in COMMON_WORDS:
        return False, f"\"{name}\" is said all the time - pick something you wouldn't say by accident."
    if len(letters) < 5:
        return True, "Short names can wake by accident. Two syllables work best (Sayso, Jarvis, Nova)."
    return True, ""
