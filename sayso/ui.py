...
from sayso.wake import name_advice as wake_name_advice
...
class TryName(ctk.CTkFrame):
    """Pick a name for Sayso, say it once, and see it answer: "Jarvis is ready to drive!"."""
    GOOD = ("#1E7F4F", "#4CC38A")
    BAD = "#E5484D"
    ...
    def __init__(self, master, ctl, **kw):
        ...
        e = ctk.CTkEntry(r, textvariable=self.name, width=200, height=36, font=font(16, "bold"))
        e.pack(side="left")
        e.bind("<Return>", lambda _: self.save())
        e.bind("<FocusOut>", lambda _: self.save())
        ctk.CTkButton(r, text="Use this name", width=120, height=36, font=font(13), fg_color=ACCENT,
                      hover_color=ACCENT_HOVER, command=self.save).pack(side="left", padx=8)
        self.advice = ctk.CTkLabel(self, text="", font=font(12), text_color=MUTED, justify="left", wraplength=400)
        self.advice.pack(anchor="w", pady=(4, 0))
        self.card = ctk.CTkFrame(self, fg_color=CARD, corner_radius=12)
        self.card.pack(fill="x", pady=(12, 0))
        self.prompt = ctk.CTkLabel(self.card, text="", font=font(15, "bold"), justify="left", wraplength=390)
        self.prompt.pack(anchor="w", padx=16, pady=(14, 2))
    ...
    def save(self):
        name = " ".join(self.name.get().split())[:30]
        ok, msg = wake_name_advice(name)
        self.advice.configure(text=msg, text_color=MUTED if ok else self.BAD)
        if ok and name != self.ctl.cfg.wake_word:
            self.ctl.update_cfg(wake_word=name)
            self._refresh()
            if self.passed:          # new name: test it again
                self.passed = False
                self._tick()

    def _refresh(self):
        n = self.ctl.cfg.wake_word
        self.prompt.configure(text=f"Now say:  \u201c{n}, hello\u201d")

    def _tick(self):
        if not self.winfo_exists() or self.passed:
            return
        ...
        if state == "loading":
            ...
            self.status.configure(text=self.ctl.last_msg or "Check your microphone.", text_color=self.BAD)
        elif state == "paused":
            self.status.configure(text=f"{APP_NAME} is paused - switch it On at the top.", text_color=MUTED)
        elif state in ("listening", "thinking"):
            self.status.configure(text="Heard something...", text_color=MUTED)
        else:
            self.status.configure(text=f"Listening for \u201c{n}\u201d...", text_color=MUTED)
        self.after(400, self._tick)

    def heard(self, action):
        n = self.ctl.cfg.wake_word
        self.passed = True
        said = action.text if action.kind in ("type", "type_send") else action.kind.replace("_", " ")
        self.prompt.configure(text=f"\u2705  {n} is ready to drive!")
...
class Wizard(ctk.CTkFrame):
    ...
    def show(self):
        ...
        self.dots.configure(text="  ".join("●" if i == self.step else "○" for i in range(4)))
        if self.step:
        ...
        if self.step == 0:
            ...
            ctk.CTkLabel(b, text="3. Name it and try it", font=font(15, "bold")).pack(anchor="w")
            ctk.CTkLabel(b, text="Call it anything - Sayso, Jarvis, Nova... Then say it once to check it hears you.",
                         font=font(12), text_color=MUTED, justify="left", wraplength=410).pack(anchor="w", pady=(2, 12))
            TryName(b, ctl).pack(fill="x")
            self.next.configure(text="Next")

        else:
            ctk.CTkLabel(b, text="4. Use it anywhere", font=font(15, "bold")).pack(anchor="w")
            ...
            wake = ctl.cfg.wake_word
            for said, does in COMMANDS[:3]:
                said = said.replace("Sayso", wake)
...
class NameDialog(ctk.CTkToplevel):
    def __init__(self, master, ctl):
        ...
        ctk.CTkLabel(self, text="Name it and try it", font=font(16, "bold")).pack(anchor="w", padx=20, pady=(16, 8))
        TryName(self, ctl).pack(fill="x", padx=20)
        ctk.CTkButton(self, text="Done", width=90, fg_color=ACCENT, hover_color=ACCENT_HOVER,
                      command=self.close).pack(anchor="e", padx=20, pady=16)
    ...
    def close(self):
        master = self.master
        self.destroy()
        if hasattr(master, "rebuild_settings"):
            master.rebuild_settings()
...
class MainWindow(ctk.CTk):
    ...
    def rebuild_settings(self):
        """After renaming, redraw the tabs that show the name."""
        if self.tabs is None:
            return
        for name, build in (("Settings", self._settings), ("Commands", self._commands)):
            tab = self.tabs.tab(name)
            for w in tab.winfo_children():
                w.destroy()
            build(tab)
        self.refresh_home()
        self.update_state(self.ctl.state, "")
    ...
    def _settings(self, t):
        ...
        def name_row(r):
            f = ctk.CTkFrame(r, fg_color="transparent")
            ctk.CTkLabel(f, text=cfg.wake_word, font=font(13, "bold")).pack(side="left", padx=(0, 8))
            ctk.CTkButton(f, text="Change & test", width=110, height=28, font=font(12), fg_color=ACCENT,
                          hover_color=ACCENT_HOVER, command=lambda: NameDialog(self, ctl)).pack(side="left")
            return f
        row("Its name (wake word)", name_row)
    ...
    def refresh_home(self):
        ...
        if hist:
            ...
            for said, does in COMMANDS[:4]:
                said = said.replace("Sayso", self.ctl.cfg.wake_word)
